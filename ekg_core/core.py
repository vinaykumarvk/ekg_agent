...
def _download_from_gcs(gcs_uri: str, dest_path: Path) -> bool:
    """
    Download a file from Google Cloud Storage to dest_path.
    """
    log = logging.getLogger(__name__)
    try:
        from google.cloud import storage
    except ImportError:
        log.warning("google-cloud-storage not installed; cannot download %s", gcs_uri)
        return False

    try:
        bucket_name, *blob_parts = gcs_uri.replace("gs://", "").split("/", 1)
        if not bucket_name or not blob_parts:
            log.error("Invalid GCS URI: %s", gcs_uri)
            return False
        blob_path = blob_parts[0]
        client = storage.Client()
        bucket = client.bucket(bucket_name)
        blob = bucket.blob(blob_path)
        dest_path.parent.mkdir(parents=True, exist_ok=True)
        blob.download_to_filename(dest_path)
        log.info("Downloaded KG from %s to %s", gcs_uri, dest_path)
        return True
    except Exception as e:
        log.error("Failed to download KG from %s: %s", gcs_uri, e)
        return False


def _resolve_kg_path(kg_path: str) -> str | None:
    """
    ...
    if p.name == "master_knowledge_graph.json":
        candidates.append(repo_root / "data" / "kg" / "master_knowledge_graph.json")
    ...
    gcs_uri = None
    if str(kg_path).startswith("gs://"):
        gcs_uri = kg_path
    elif p.name == "master_knowledge_graph.json":
        gcs_uri = os.getenv("MASTER_KG_GCS_URI")
    ...
    log.error("KG path could not be resolved: %s", kg_path)
    return None
